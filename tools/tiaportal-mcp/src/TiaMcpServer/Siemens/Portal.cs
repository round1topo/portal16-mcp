...
namespace TiaMcpServer.Siemens
{
    public partial class Portal
    {
        ...
        private Project? _softwareCacheProject;
        private readonly ILogger<Portal>? _logger;
        private StaExecutor _sta = new StaExecutor();
        ...
        public void Dispose()
        {
            try
            {
                _sta.Run(() => (_project as Project)?.Close());
            }
            catch (Exception ex)
            {
        ...
        private TiaPortal? AttachWithTimeout(int processId, int timeoutMs, out StaExecutor? portalSta)
        {
            // The attach call can block for minutes. Give each attempt its own persistent
            // STA so a timed-out attempt can be abandoned without poisoning the active Portal STA.
            var attemptSta = new StaExecutor();
            ...
            var worker = new System.Threading.Thread(() =>
            {
                try
                {
                    result = attemptSta.Run<TiaPortal>(() =>
                    {
                        var process = TiaPortal.GetProcesses()
                            .FirstOrDefault(item => item.Id == processId);
                        if (process == null)
                        {
                            throw new PortalException(
                                PortalErrorCode.OpennessError,
                                $"TIA Portal process PID={processId} disappeared before Attach().");
                        }
            ...
            {
                IsBackground = true,
                Name = $"PortalAttach-{processId}"
            };
            worker.Start();
            if (!worker.Join(timeoutMs))
            {
                _logger?.LogWarning($"Attach to TIA Portal PID={processId} exceeded {timeoutMs}ms; abandoning this isolated STA attempt.");
                // Attach cannot be cancelled safely. Do not dispose attemptSta here: its
                // dispatcher may still own the blocked COM call. The background STA is
                // isolated from the active Portal STA and will die with the MCP process.
                portalSta = null;
                return null;
            }
        ...
        public bool ConnectPortal()
        {
            ...
            try
            {
                ...
                // Only ordinary process IDs leave the persistent STA. The surrounding
                // connection workflow stays off that STA so each isolated Attach attempt
                // can wait without blocking subsequent MCP calls.
                var selectedProcessId = Engineering.TiaPortalProcessId;
                var processIds = _sta.Run(() => TiaPortal.GetProcesses().Select(proc => proc.Id).ToList());
                if (selectedProcessId.HasValue)
                {
                    var requestedPid = selectedProcessId.Value;
                    processIds = processIds
                        .Where(processId => processId == requestedPid)
                        .ToList();
                    if (processIds.Count == 0)
                    {
                        throw new PortalException(
                            PortalErrorCode.OpennessError,
                            $"Requested TIA Portal process PID={requestedPid} was not found.");
                    }

                    _logger?.LogInformation($"Restricting connection to requested TIA Portal PID={requestedPid}");
                }

                var hasExplicitProcessSelection = selectedProcessId.HasValue;
                _logger?.LogInformation($"TIA Portal process count: {processIds.Count}");
                bool noAttach = string.Equals(
                    Environment.GetEnvironmentVariable("TIA_MCP_NO_ATTACH"),
                    "1",
                ...
                if (noAttach)
                {
                    _logger?.LogInformation("TIA_MCP_NO_ATTACH is set; skipping existing Portal processes.");
                }
                else if (processIds.Any())
                {
                    // IMPORTANT: multiple Siemens.Automation.Portal.exe can run at once.
                    // Try each process and prefer the one with an open project.
                    TiaPortal? firstAttachable = null;
                    StaExecutor? firstAttachableSta = null;
                    string? firstAttachableInfo = null;

                    foreach (var processId in processIds)
                    {
                        TiaPortal? candidate = null;
                        StaExecutor? candidateSta = null;
                        try
                        {
                            _logger?.LogInformation($"Trying attach to TIA Portal process PID={processId}");
                            candidate = AttachWithTimeout(processId, 30000, out candidateSta);
                            _logger?.LogInformation(candidate == null
                                ? $"Attach returned null/timed out for PID={processId} — skipping"
                                : $"Attach succeeded for PID={processId}");
                            if (candidate == null) continue;

                            if (firstAttachable == null)
                            {
                                firstAttachable = candidate;
                                firstAttachableSta = candidateSta;
                                firstAttachableInfo = $"PID={processId}";
                            }

                            bool hasProject = false;
                            try
                            {
                                hasProject = candidateSta != null && candidateSta.Run(() => candidate.Projects.Any());
                            }
                            catch { }
                            _logger?.LogInformation($"Portal PID={processId}: hasProject={hasProject}");

                            if (hasProject)
                            {
                                _portal = candidate;
                                _logger?.LogInformation($"Selected attached TIA Portal PID={processId}");
                                try { _project = candidateSta!.Run(() => candidate.Projects.First()); } catch { }
                                AdoptPortalSta(candidateSta!);
                        ...
                        {
                            _logger?.LogWarning(ex, $"Attach failed for TIA Portal PID={processId}");
                            LastConnectError = ex.ToString();
                        }
                        finally
                        {
                            if (candidate != null && candidate != _portal && candidate != firstAttachable && candidateSta != null)
                            {
                                try { candidateSta.Run(() => candidate.Dispose()); } catch { }
                                try { candidateSta.Dispose(); } catch { }
                            }
                        }
                    }
                    ...
                    if (firstAttachable != null)
                    {
                        ...
                        AdoptPortalSta(firstAttachableSta!);
                        _logger?.LogInformation($"Falling back to first attachable TIA Portal ({firstAttachableInfo})");
                        LastConnectError = $"Attached to first available portal ({firstAttachableInfo}), but it has no visible projects.";
                        return true;
                    }
                    ...
                    if (hasExplicitProcessSelection)
                    {
                        throw new PortalException(
                            PortalErrorCode.OpennessError,
                            $"Unable to attach to requested TIA Portal process PID={selectedProcessId.GetValueOrDefault()} within the connection timeout.");
                    }
                ...
                // Start new TIA Portal on the same persistent STA used by all later calls.
                ...
                _portal = _sta.Run(() => new TiaPortal(launchMode));
        ...
        private void AdoptPortalSta(StaExecutor portalSta)
        {
            if (ReferenceEquals(_sta, portalSta))
            {
                return;
            }

            var previousSta = _sta;
            _sta = portalSta;
            try { previousSta.Dispose(); } catch { }
        }

        public List<string> ListPortalProcessIds()
        {
            return _sta.Run(() =>
            {
        ...
        public bool AttachToOpenProject(string projectName)
        {
            ...
            // Connect 之后 TIA 的 Projects 是异步填充的，
        ...
        public bool OpenProject(string projectPath)
        {
            if (!_sta.IsStaThread)
            {
                return _sta.Run(() => OpenProject(projectPath));
            }
        ...
        public List<Project> GetSessions()
        {
        ...
        public bool OpenSession(string localSessionPath)
        {
            ...
            _logger?.LogWarning("Local sessions are not supported in the V16 compatibility build.");
            return false;
        }
        ...
        public bool SaveSession()
        {
            ...
            _logger?.LogWarning("SaveSession is not supported in the V16 compatibility build.");
            return true;
        }
        ...
        public bool CloseSession()
        {
            ...
            _logger?.LogWarning("CloseSession is not supported in the V16 compatibility build.");
            return false;
        }
        ...
        public List<ResponseDeviceInfo> GetDeviceInfos(string regexName = "")
        {
            if (_sta.IsStaThread)
                return GetDeviceInfosCore(regexName);

            return _sta.Run(() => GetDeviceInfosCore(regexName));
        }

        private List<ResponseDeviceInfo> GetDeviceInfosCore(string regexName = "")
        {
            var responseList = new List<ResponseDeviceInfo>();
            foreach (var device in GetDevices(regexName))
            {
                if (device == null)
                    continue;
        ...
        {
            if (_sta.IsStaThread)
                return AddDeviceCore(orderNumber, version, deviceName);

            return _sta.Run(() => AddDeviceCore(orderNumber, version, deviceName));
        }

        private Device AddDeviceCore(string orderNumber, string version, string deviceName)
        {
        ...
        public string AddDeviceByTypeIdentifier(string typeIdentifier, string deviceName, string itemName = "")
        {
            if (_sta.IsStaThread)
                return AddDeviceByTypeIdentifierCore(typeIdentifier, deviceName, itemName);

            return _sta.Run(() => AddDeviceByTypeIdentifierCore(typeIdentifier, deviceName, itemName));
        }

        private string AddDeviceByTypeIdentifierCore(string typeIdentifier, string deviceName, string itemName = "")
        {
            if (IsProjectNull())
                throw new PortalException(PortalErrorCode.InvalidState, "No project is open in TIA Portal");

            var normalizedTypeIdentifier = (typeIdentifier ?? string.Empty).Trim();
            if (string.IsNullOrWhiteSpace(normalizedTypeIdentifier))
                throw new PortalException(PortalErrorCode.InvalidParams, "TypeIdentifier is empty");

            var normalizedDeviceName = (deviceName ?? string.Empty).Trim();
            if (string.IsNullOrWhiteSpace(normalizedDeviceName))
                throw new PortalException(PortalErrorCode.InvalidParams, "Device name is empty");

            var project = _project as Project;
            if (project == null)
                throw new PortalException(PortalErrorCode.InvalidState, "Current project is not a local Project instance");

            var normalizedItemName = string.IsNullOrWhiteSpace(itemName)
                ? MakeEngineeringName(normalizedDeviceName)
                : itemName.Trim();

            Exception? createWithItemError = null;
            try
            {
                var deviceWithItems = project.Devices.CreateWithItem(
                    normalizedTypeIdentifier,
                    normalizedItemName,
                    normalizedDeviceName);
                if (deviceWithItems is Device createdWithItems)
                    return createdWithItems.Name;
            }
            catch (Exception ex)
            {
                createWithItemError = ex;
            }

            try
            {
                var device = project.Devices.Create(normalizedTypeIdentifier, normalizedDeviceName);
                if (device is Device createdDevice)
                    return createdDevice.Name;
            }
            catch (Exception ex)
            {
                var first = createWithItemError == null
                    ? string.Empty
                    : $" CreateWithItem: {FormatExceptionDetail(createWithItemError)}";
                throw new PortalException(
                    PortalErrorCode.OpennessError,
                    $"Device creation failed for TypeIdentifier '{normalizedTypeIdentifier}'.{first} Create: {FormatExceptionDetail(ex)}",
                    null,
                    ex);
            }
            ...
            throw new PortalException(
                PortalErrorCode.OpennessError,
                $"Device creation returned no device for TypeIdentifier '{normalizedTypeIdentifier}'");
        }
        ...
        {
            if (_sta.IsStaThread)
                return AddDeviceWithFallbackCore(preferredMlfb, preferredVersion, deviceName, family);

            return _sta.Run(() => AddDeviceWithFallbackCore(preferredMlfb, preferredVersion, deviceName, family));
        }

        private (Device? Device, string? MlfbUsed, string? VersionUsed, List<string> Attempts, string? Error) AddDeviceWithFallbackCore(
            string preferredMlfb,
            string preferredVersion,
            string deviceName,
            string family)
        ...
        {
            if (_sta.IsStaThread)
                return SearchHardwareCatalogCore(keyword, limit);

            return _sta.Run(() => SearchHardwareCatalogCore(keyword, limit));
        }

        private List<HardwareCatalogCandidate> SearchHardwareCatalogCore(string keyword, int limit = 50)
        {
            var normalizedKeyword = (keyword ?? string.Empty).Trim();
        ...
        {
            if (_sta.IsStaThread)
                return AddHardwareCatalogDeviceWithProbeCore(keyword, deviceName, preferredText);

            return _sta.Run(() => AddHardwareCatalogDeviceWithProbeCore(keyword, deviceName, preferredText));
        }

        private (Device? Device, HardwareCatalogCandidate? Candidate, List<HardwareCatalogCandidate> Candidates, List<string> Attempts, string? Error)
            AddHardwareCatalogDeviceWithProbeCore(string keyword, string deviceName, string preferredText = "")
        {
            var attempts = new List<string>();
        ...
        public ResponseDeviceItemInfo? GetDeviceItemInfoSnapshot(string deviceItemPath)
        {
            if (_sta.IsStaThread)
                return GetDeviceItemInfoSnapshotCore(deviceItemPath);

            return _sta.Run(() => GetDeviceItemInfoSnapshotCore(deviceItemPath));
        }

        private ResponseDeviceItemInfo? GetDeviceItemInfoSnapshotCore(string deviceItemPath)
        {
            ...
            return new ResponseDeviceItemInfo
            {
                ...
                Attributes = Helper.GetAttributeList(deviceItem),
        ...
        public ModelContextProtocol.ResponseSoftwareInfo? GetPlcSoftwareInfo(string softwarePath)
        {
            return _sta.Run(() =>
            {
                _logger?.LogInformation($"Getting software info by path: {softwarePath}");
                ...
                return new ModelContextProtocol.ResponseSoftwareInfo
                {
                    Message = $"Software info retrieved from '{softwarePath}'",
                    Name = plcSoftware.Name,
                    Attributes = ModelContextProtocol.Helper.GetAttributeList(plcSoftware),
                    Description = plcSoftware.ToString(),
        ...
        {
            _sta.Run(() => ImportPlcTagTableCore(softwarePath, folderPath, importPath));
        }

        private void ImportPlcTagTableCore(string softwarePath, string folderPath, string importPath)
        {
            if (IsProjectNull()) throw new PortalException(PortalErrorCode.InvalidState, "No project is open in TIA Portal");
        ...
        {
            return _sta.Run(() => GetPlcExternalSourcesCore(softwarePath));
        }

        private List<string>? GetPlcExternalSourcesCore(string softwarePath)
        {
            if (IsProjectNull()) return null;
            var softwareContainer = GetSoftwareContainer(softwarePath);
        ...
        {
            _sta.Run(() => DeletePlcExternalSourceCore(softwarePath, externalSourceName));
        }

        private void DeletePlcExternalSourceCore(string softwarePath, string externalSourceName)
        {
            if (IsProjectNull())
        ...
        {
            _sta.Run(() => ImportPlcExternalSourceCore(softwarePath, groupPath, filePath));
        }

        private void ImportPlcExternalSourceCore(string softwarePath, string groupPath, string filePath)
        {
            if (IsProjectNull())
            ...
            // V16 exposes a stable strongly typed CreateFromFile entry point. Prefer it
            // over reflection because a reflected composition proxy can become disposed
            // before the COM call is made.
            try
            {
                if (group is PlcExternalSourceSystemGroup systemGroup)
                {
                    var created = systemGroup.ExternalSources.CreateFromFile(fi.Name, fi.FullName);
                    if (created != null) return;
                }
                else if (group is PlcExternalSourceGroup userGroup)
                {
                    var created = userGroup.ExternalSources.CreateFromFile(fi.Name, fi.FullName);
                    if (created != null) return;
                }
            }
            catch (Exception ex)
            {
                _logger?.LogDebug(ex, "V16 strongly typed PLC external source import failed; trying reflection fallback.");
            }
        ...
        {
            _sta.Run(() => GenerateBlocksFromExternalSourceCore(softwarePath, externalSourceName));
        }

        private void GenerateBlocksFromExternalSourceCore(string softwarePath, string externalSourceName)
        {
        ...
        public CompilerResult CompileSoftware(string softwarePath, string password = "", int timeoutSeconds = 120)
        {
            if (_sta.IsStaThread)
                return CompileSoftwareCore(softwarePath, password);

            var task = System.Threading.Tasks.Task.Factory.StartNew(
                () => _sta.Run(() => CompileSoftwareCore(softwarePath, password)),
            ...
            try
            {
                ...
                {
                    _logger?.LogWarning(
                        $"Compile of '{softwarePath}' exceeded {timeoutSeconds}s; the STA operation remains running.");
                    throw new PortalException(
                        PortalErrorCode.InvalidState,
                        $"Compile exceeded {timeoutSeconds}s timeout for '{softwarePath}'. " +
                        "底层编译仍在 TIA Portal STA 线程中运行，不能强制取消；请等待其完成后再重试。");
                }
        ...
        /// <summary>
        /// 在 PortalSta 线程读取编译结果，并只向线程外返回普通托管对象。
        /// CompilerResult 及其 Messages 属于 Openness COM 对象，不能在 MCP 调度线程读取。
        /// </summary>
        public CompilerResultSnapshot CollectCompilerResultOnSta(object? result)
        {
            if (result == null)
                return new CompilerResultSnapshot();

            return _sta.Run(() =>
            {
                var snapshot = new CompilerResultSnapshot();
                var resultType = result.GetType();

                try
                {
                    snapshot.State = resultType.GetProperty("State")?.GetValue(result)?.ToString() ?? "";
                }
                catch
                {
                }
                ...
                try
                {
                    snapshot.ErrorCount = ReadCompilerIntProperty(result, "ErrorCount");
                }
                catch
                {
                }
                ...
                try
                {
                    snapshot.WarningCount = ReadCompilerIntProperty(result, "WarningCount");
                }
                catch
                {
                }
                ...
                try
                {
                    var messages = resultType.GetProperty("Messages")?.GetValue(result);
                    var collected = McpServer.CollectCompilerMessages(messages);
                    snapshot.Raw = collected.Raw;
                    snapshot.Errors = collected.Errors;
                    snapshot.Warnings = collected.Warnings;
                    snapshot.Info = collected.Info;
                }
                catch
                {
                }
        ...
        private static int ReadCompilerIntProperty(object result, string propertyName)
        {
            var raw = result.GetType().GetProperty(propertyName)?.GetValue(result);
            if (raw is int value)
                return value;
        ...
        {
#if TIA_V16
            return new ResponseMessage { Message = "TIA Portal V16 不支持报警类别导出。" };
#else
            if (IsProjectNull()) return new ResponseMessage { Message = "No project open." };
            var plc = GetPlcSoftware(softwarePath);
            if (plc == null) return new ResponseMessage { Message = $"PLC software not found: '{softwarePath}'." };
        ...
        {
#if TIA_V16
            return new ResponseMessage { Message = "TIA Portal V16 不支持报警类别导入。" };
#else
            if (IsProjectNull()) return new ResponseMessage { Message = "No project open." };
            var plc = GetPlcSoftware(softwarePath);
            if (plc == null) return new ResponseMessage { Message = $"PLC software not found: '{softwarePath}'." };
        ...
        public ModelContextProtocol.BlockGroupInfo? GetBlocksWithHierarchy(string softwarePath)
        {
            return _sta.Run(() =>
            {
                if (IsProjectNull())
                ...
                return rootGroup == null
                    ? null
                    : ModelContextProtocol.Helper.BuildBlockHierarchy(rootGroup);
            });
        }
        ...
        {
#if TIA_V16
            _logger?.LogWarning("ExportAsDocuments is not supported on TIA Portal V16.");
            return false;
#else
        ...
        {
#if TIA_V16
            _logger?.LogWarning("ExportBlocksAsDocuments is not supported on TIA Portal V16.");
            return null;
#else
        ...
        public bool ImportFromDocuments(string softwarePath, string groupPath, string importPath, string fileNameWithoutExtension, ImportDocumentOptions option)
        {
            ...
            _logger?.LogWarning("ImportFromDocuments is not supported in the V16 compatibility build.");
            return false;
        }
        ...
        public IEnumerable<PlcBlock>? ImportBlocksFromDocuments(string softwarePath, string groupPath, string importPath, string regexName, ImportDocumentOptions option, bool preservePath = false)
        {
            ...
            _logger?.LogWarning("ImportBlocksFromDocuments is not supported in the V16 compatibility build.");
            return null;
        }
        ...
        {
            _logger?.LogWarning("Session support is disabled in the V16 compatibility build.");
            return true;
        }
    ...
    /// <summary>
    /// 编译结果的普通托管快照，避免 Openness COM 对象跨线程使用。
    /// </summary>
