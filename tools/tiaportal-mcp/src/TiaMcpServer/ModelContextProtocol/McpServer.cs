...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
        ...
        [McpServerTool(Name = "ListPortalProcessIds"), Description("[L1][Portal]List running TIA Portal process IDs without attaching to or modifying any process.")]
        public static ResponseStringList ListPortalProcessIds()
        {
            try
            {
                var items = Portal.ListPortalProcessIds();
                return new ResponseStringList
                {
                    Message = "TIA Portal process IDs inspected",
                    Items = items,
                    Meta = new JsonObject
                    {
                        ["timestamp"] = DateTime.Now,
            ...
            {
                throw new McpException($"Unexpected error listing TIA Portal process IDs: {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }
            ...
            try
            {
                ...
                var layers = new BootstrapToolLayers
                {
                    ...
                    L1 = new[]
                    {
                        ...
                        "PlcBuildAndImport", "CompileSoftware"
                    },
            ...
            [Description("deviceItemPath: defines the path in the project structure to the device item")] string deviceItemPath)
        {
            try
            {
                var snapshot = Portal.GetDeviceItemInfoSnapshot(deviceItemPath);
                if (snapshot != null)
                {
                    return snapshot;
                }
        ...
        public static ResponseDevices GetDevices()
        {
            try
            {
                var responseList = Portal.GetDeviceInfos();
            ...
            [Description("deviceName: name in project tree")] string deviceName)
        {
            try
            {
                Portal.AddDevice(orderNumber, version, deviceName);
                return new ResponseMessage
                {
        ...
        [McpServerTool(Name = "AddDeviceByTypeIdentifier"), Description("[L1][Hardware] Add one device with an exact Siemens Openness TypeIdentifier already known by the caller. This is the V16-safe path when HardwareCatalog is unavailable. Requires: Connect + CreateProject or OpenProject. Use a TypeIdentifier read from a real V16 project tree or another verified V16 source.")]
        public static ResponseMessage AddDeviceByTypeIdentifier(
            [Description("typeIdentifier: exact Openness TypeIdentifier, e.g. 'System:Device.S71500'")] string typeIdentifier,
            [Description("deviceName: name in project tree")] string deviceName,
            [Description("itemName: optional station item name; empty uses a generated name")] string itemName = "")
        {
            try
            {
                var createdName = Portal.AddDeviceByTypeIdentifier(typeIdentifier, deviceName, itemName);
                return new ResponseMessage
                {
                    Message = $"Device '{createdName}' added by TypeIdentifier",
                    Meta = new JsonObject
                    {
                        ["timestamp"] = DateTime.Now,
                        ["success"] = true,
            ...
            {
                throw new McpException($"Failed to add device '{deviceName}' by TypeIdentifier '{typeIdentifier}' [{pex.Code}]: {pex.Message}", pex, McpErrorCode.InternalError);
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw new McpException($"Unexpected error adding device by TypeIdentifier: {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }
            ...
            [Description("family: fallback hint, e.g. 'S7-1200', 'S7-1500', or 'WinCCUnifiedPC'")] string family = "S7-1500")
        {
            try
            {
                ...
                return new ResponseDeviceProbe
                {
                    ...
                    Attempts = res.Attempts,
                    Meta = new JsonObject { ["timestamp"] = DateTime.Now, ["success"] = true, ["name"] = deviceName }
                };
            }
            catch (Exception ex) when (ex is not McpException)
            {
            ...
            [Description("softwarePath: defines the path in the project structure to the plc software")] string softwarePath)
        {
            try
            {
                var response = Portal.GetPlcSoftwareInfo(softwarePath);
                if (response != null)
                {
                    return response;
                }
        ...
        {
            var snapshot = Portal.CollectCompilerResultOnSta(result);
            return new ResponseCompile
            {
                Message = $"Software '{softwarePath}' compiled. State={snapshot.State} Errors={snapshot.ErrorCount} Warnings={snapshot.WarningCount}",
                State = snapshot.State,
                ErrorCount = snapshot.ErrorCount,
                WarningCount = snapshot.WarningCount,
                Messages = snapshot.Raw,
                Meta = new JsonObject
                {
                    ["timestamp"] = DateTime.Now,
                    ["success"] = !snapshot.State.Equals("Error", StringComparison.OrdinalIgnoreCase),
                    ["errorDetailCount"] = snapshot.Errors.Count,
                    ["warningDetailCount"] = snapshot.Warnings.Count
                }
            };
        }
            ...
            try
            {
                ...
                if (block != null)
                {
                    ...
                    return new ResponseBlockInfo
                    {
                        ...
                        Namespace = Siemens.V16CompatShim.GetOptionalNamespace(block),
        ...
        [Description("softwarePath: defines the path in the project structure to the plc software")] string softwarePath)
        {
            try
            {
                var hierarchy = Portal.GetBlocksWithHierarchy(softwarePath);
                if (hierarchy != null)
                {
                    ...
                    foreach (var b in allBlocks)
                    {
                        if (b != null && b.IsConsistent == false)
                        {
                            ...
                            inconsistentInfos.Add(new ResponseBlockInfo
                            {
                                ...
                                Namespace = Siemens.V16CompatShim.GetOptionalNamespace(b),
            ...
            try
            {
                ...
                if (type != null)
                {
                    ...
                    return new ResponseTypeInfo
                    {
                        ...
                        Namespace = Siemens.V16CompatShim.GetOptionalNamespace(type),
                    ...
                    foreach (var t in allTypes)
                    {
                        if (t != null && t.IsConsistent == false)
                        {
                            ...
                            inconsistentTypeInfos.Add(new ResponseTypeInfo
                            {
                                ...
                                Namespace = Siemens.V16CompatShim.GetOptionalNamespace(t),
